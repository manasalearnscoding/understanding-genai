...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("--llama_path", type=str, default=None, help="path to llama2 model checkpoints")
    parser.add_argument("--include_negatives", action="store_true", help="use if want to run tracing for incorrectly predicted examples as well")
    parser.add_argument(
        "--model",
        required=True,
        choices=[
    ...
    parser.add_argument("--winograd_file", type=str, default="data/anti_stereotyped_type1.txt.dev", help="path to Winograd dataset file")
    parser.add_argument("--k", type=int, default=10, help="top k tokens to save when performing vocabulary projection")
    parser.add_argument("--num_instances", default=5000, type=int)
    parser.add_argument("--override", help="use this to allow overwrites of output files", action="store_true")
    parser.add_argument("--no_model_load", action="store_true", help="use to speed up debugging by not loading the model checkpoint")
    args = parser.parse_args()
    
    if "llama" in args.model and args.llama_path is None: #try run on non laama model, need to specify checkpoint?
        #check args.model? check after running
        raise Exception("Need a path to load Huggingface llama model checkpoints")
    
    print(f"Running {args.model} on Winograd dataset")
    ...
    knowns = read_winocoref_file(args.winograd_file)
    
    # Subset if needed
    if len(knowns) > args.num_instances:
        print(f"Subsetting down dataset to {args.num_instances} datapoints")
        random.seed(10)
        knowns = random.sample(knowns, args.num_instances)
    ...
    # Setup file paths CHECK
    # records_filepath = f"./results/memit_outputs/winograd/{args.model}/files/val/"
    # if not os.path.exists(records_filepath):
    #     os.makedirs(records_filepath)
    
    # Set up filename CHECK
    # if "trace" in args.function:
    #     records_filename = os.path.join(records_filepath, f"winograd_causalTrace_{kind}_results.jsonl")
    # elif "calculate_resolution_acc" in args.function:
    #     records_filename = os.path.join(records_filepath, "winograd_resolution_acc.csv")
    # elif "vocab_projection" in args.function:
    #     records_filename = os.path.join(records_filepath, f"winograd_{args.function}_top{args.k}.jsonl")
    
    # Check if output file already exists CHECK
    # already_completed = 0
    # if not args.override and os.path.exists(records_filename):
    #     already_completed = sum(1 for _ in open(records_filename))
    #     if already_completed >= len(knowns):
    #         raise Exception("Complete file already exists")
    #     else:
    #         print(f"Completing {len(knowns)-already_completed} instances")
    # 
    # print(f"WRITING TO {records_filename}")
    
    times = []
    traced = 0
    
    # Main processing loop
    for i, item in tqdm(enumerate(knowns[already_completed:], start=already_completed), total=len(knowns) - already_completed):
        start_time = time.time()
        
        # Tokenize the sentence
        inp = make_inputs(mt.tokenizer, [item['sentence']])
        
        # Get token positions for entities and pronoun
        entity1_tokens = mt.tokenizer.encode(item['entity1'], add_special_tokens=False)
        entity2_tokens = mt.tokenizer.encode(item['entity2'], add_special_tokens=False)
        pronoun_tokens = mt.tokenizer.encode(item['pronoun'], add_special_tokens=False)
        
        all_tokens = inp["input_ids"][0].tolist()
        entity1_pos = find_subsequence_position(all_tokens, entity1_tokens)
        entity2_pos = find_subsequence_position(all_tokens, entity2_tokens)
        pronoun_pos = find_subsequence_position(all_tokens, pronoun_tokens)
        
        if "trace" in args.function:
            # Perform causal tracing
            tracing_result = trace_with_patch(
                mt=mt,
                inp=inp,
                answers_t=[entity1_pos, entity2_pos, pronoun_pos, None],
                indices_to_replace=[pronoun_pos],
                kind=kind,
            )
            ...
            result = {
                ...
                'correct_referent': item['correct_referent'],
            ...
            with jsonlines.open(records_filename, mode='a') as writer:
                writer.write(result)
            
            traced += 1
            
        elif "calculate_resolution_acc" in args.function:
            # Calculate pronoun resolution accuracy
            with torch.inference_mode():
                outputs = mt.model(input_ids=inp["input_ids"], return_dict=True)
            
            # Extract logits at pronoun position
            pronoun_pos_logits = outputs.logits[0, pronoun_pos, :]
            pronoun_pos_probs = torch.nn.Softmax(dim=0)(pronoun_pos_logits)
            
            # Get probabilities for entity1 and entity2 tokens
            entity1_first_token = entity1_tokens[0]
            entity2_first_token = entity2_tokens[0]
            
            entity1_prob = pronoun_pos_probs[entity1_first_token].item()
            entity2_prob = pronoun_pos_probs[entity2_first_token].item()
            
            # Determine prediction
            predicted_referent = item['entity1'] if entity1_prob > entity2_prob else item['entity2']
            correct = predicted_referent == item['correct_referent']
            
            # Write to CSV
            if i == already_completed:
                with open(records_filename, 'w' if args.override else 'a') as f:
                    writer = csv.writer(f)
                    if args.override or already_completed == 0:
                        writer.writerow([
                            'index', 'sentence', 'entity1', 'entity2', 'pronoun',
                            'correct_referent', 'predicted_referent', 'entity1_prob',
                            'entity2_prob', 'correct'
                        ])
            
            with open(records_filename, 'a') as f:
                writer = csv.writer(f)
                writer.writerow([
                    item['index'], item['sentence'], item['entity1'], item['entity2'],
                    item['pronoun'], item['correct_referent'], predicted_referent,
                    entity1_prob, entity2_prob, correct
                ])
        
        elif "vocab_projection" in args.function:
            # Vocabulary projection logic for Winograd
            # TODO: Implement vocab projection specific to Winograd task
            pass
    ...
    print(f"AVERAGE LOOP TIME: {round(sum(times) / len(times) if times else 0, 2)} minutes")
    print(f"TOTAL SCRIPT TIME: {round((time.time() - og_start_time) / 60.0 / 60.0, 2)} hours")